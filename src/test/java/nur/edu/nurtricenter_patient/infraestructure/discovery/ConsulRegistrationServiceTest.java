...
class ConsulRegistrationServiceTest {
	...
	void buildDefinition_producesConsulPayloadWithCheck() {
		...
		props.setServiceAddress("207.180.197.169");
		...
		assertThat(payload.get("Address")).isEqualTo("207.180.197.169");
		...
		assertThat(check.get("HTTP")).isEqualTo("http://207.180.197.169:8080/actuator/health");
	...
	void baseUri_concatenatesSchemeHostPort() {
		...
		props.setHost("207.180.197.169");
		props.setPort(8500);

		assertThat(props.baseUri()).isEqualTo("http://207.180.197.169:8500");
	}
}
